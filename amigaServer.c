...
int32_t RecvData(SOCKET s, void *buffer, uint32_t len, int flags)
{
	...
	while(len)
	{
		int32_t rlen = recv(s, data + pos, len, 0);
		if(!rlen)
			return 0;
		if(rlen == -1)
		{
...
int32_t ReceiveFile(SOCKET s)
{
	...
	RecvData(s, &fileLen, sizeof(fileLen), 0);
	fileLen = ntohl(fileLen);
	printf("Recieving : %s (%d)\n", fileName, fileLen);
	fileData = malloc(fileLen + 1);
	if(!fileData)
	{
		printf("No room for file data\n");
		free(fileName);		
		return 0;
	}

	RecvData(s, fileData, fileLen, 0);
	fileData[fileLen] = 0;
...
int32_t ReceiveFileCRC(SOCKET s)
{
	int32_t nameLen;
	int32_t fileLen;
	char *fileName;
	...
	uint32_t crc32;
	RecvData(s, &crc32, sizeof(crc32), 0);
	crc32 = ntohl(crc32);

	RecvData(s, fileData, fileLen, 0);
	fileData[fileLen] = 0;

	uint32_t computedCRC32 = CRC32((uint8_t *)fileData, fileLen);
...
int ExecuteCommand(SOCKET s)
{
	int32_t commandLen;
	char *command;
	RecvData(s, &commandLen, sizeof(commandLen), 0);
...
int ExecuteCommandOutput(SOCKET s)
{
	int32_t commandLen;
	char *command;
	RecvData(s, &commandLen, sizeof(commandLen), 0);
...
void ProcessCommands(SOCKET s)
{
	...
	while(running)
	{
		int32_t command = COMMAND_CLOSE;
		res = RecvData(s, &command, sizeof(command), 0);
...
int main(int argc, char *argv[])
{
	...
	if(!SocketBase)
		SocketBase = (struct Library *)OpenLibrary("bsdsocket.library", 4UL);
	if(!SocketBase)
	{
		printf("No TCP stack!\n");
		return 1;
